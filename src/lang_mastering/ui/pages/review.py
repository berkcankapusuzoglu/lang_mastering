...
def review_page(page: ft.Page) -> ft.View:
    ...
    lesson = page.session.store.get("current_lesson")
    ...
    if lesson and len(session_cards) < 20:
        ...
        if page.session.store.contains_key("current_lesson"):
            page.session.store.remove("current_lesson")
    ...
    def show_summary():
        ...
        page.session.store.set("current_user", user)
        ...
        exercise_container.content = ft.Column([
            ...
            ft.ElevatedButton(
                "Back to Home",
                icon=ft.Icons.HOME,
                on_click=lambda _: page.session.store.get("router").navigate("/"),
                style=ft.ButtonStyle(bgcolor=ACCENT_COLOR, color=TEXT_COLOR),
                width=200,
            ),
        ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, alignment=ft.MainAxisAlignment.START)
